...
{
  // TODO
  // y 값이 양수인 경우는 스크롤 업 (줌 인)
  // y 값이 음수인 경우는 스크롤 다운 (줌 아웃)
  float zoomSpeed = 1.0f; // 줌 속도 조절

  // 카메라의 현재 모드에 따라 처리
  if (g_camera.mode() == Camera::kPerspective)
  {
    // 퍼스펙티브 모드인 경우 fovy 값을 조정하여 줌 인/아웃
    float fovy = g_camera.fovy();
    fovy += (float)y * zoomSpeed;
    fovy = glm::clamp(fovy, 10.f, 160.f); // 최소값과 최대값 설정 가능
    g_camera.set_fovy(fovy);
  }
  else
  {
    // 오소 그래픽 모드인 경우 ortho_scale 값을 조정하여 줌 인/아웃
    float ortho_scale = g_camera.ortho_scale();
    ortho_scale += (float)y * zoomSpeed;
    ortho_scale = glm::clamp(ortho_scale, 0.1f, 10.f); // 최소값과 최대값 설정 가능
    g_camera.set_ortho_scale(ortho_scale);
  }
}
