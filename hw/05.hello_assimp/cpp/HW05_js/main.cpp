...
void save_file(const std::string& filename)
{
  ...
  for (int i = 0; i < objects.size(); i++){
    glm::vec3 tmp = objects[i].translate();
    f << object_names[i] << '\n' << objects[i].scale()[0] << '\n';
    f << tmp[0] << ' ' << tmp[1] << ' ' << tmp[2] << '\n';
  }
  ...
  for (int i = 0; i < cameras.size(); i++){
    glm::vec3 camTemp = cameras[i].position();
    for (int k = 0; k < 3; ++k)
      f << camTemp[k] << ' ';
    f << '\n';

    camTemp = cameras[i].front_direction();
    for (int k = 0; k < 3; ++k)
      f << camTemp[k] << ' ';
    f << '\n';

    camTemp = cameras[i].up_direction();
    for (int k = 0; k < 3; ++k)
      f << camTemp[k] << ' ';
    f << '\n';
  }
...
void render_object()
{
  ...
  if(g_is_perspective==false){
    cameras[cam_select_idx].set_mode(Camera::kOrtho);
    mat_proj = glm::ortho(-1.0f, 1.0f, -1.0f, 1.0f, 0.1f, 5.0f);
  }
  if(g_is_perspective==true){
    cameras[cam_select_idx].set_mode(Camera::kPerspective);
    mat_proj = glm::perspective(glm::radians(cameras[cam_select_idx].fovy()), 1.0f, 0.1f, 5.0f);
  }

  mat_model = objects[obj_select_idx].get_model_matrix();
  // 특정 쉐이더 프로그램 사용
