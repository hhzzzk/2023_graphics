...
void Object::init_buffer_objects() 
{
   ...
   glGenBuffers(1, &position_buffer);
    ...
    for (unsigned int i = 0; i < pmesh_->mNumFaces; i++) {
        Face tmp;
        tmp.num_indices = pmesh_->mFaces[i].mNumIndices;
        glGenBuffers(1, &tmp.index_buffer);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, tmp.index_buffer);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, tmp.num_indices * sizeof(unsigned int), pmesh_->mFaces[i].mIndices, GL_STATIC_DRAW);
        faces.push_back(tmp);
    }
}
